...
HLO_BACKEND = converters.TFLiteBackend.HLO
JAX2TF_BACKEND = converters.TFLiteBackend.JAX2TF

CONVERTERS = {
    'jax2tf_xla':
        converters.jax2tf_fn(enable_xla=True),
    'jax2tf_no_xla':
        converters.jax2tf_fn(enable_xla=False),
    'jax2tfjs':
        converters.jax2tfjs,
    'jax2tflite':
        converters.jax2tflite_fn(JAX2TF_BACKEND, use_flex_ops=False),
    'jax2tflite+flex':
        converters.jax2tflite_fn(JAX2TF_BACKEND, use_flex_ops=True),
    'convert_hlo':
        converters.jax2tflite_fn(HLO_BACKEND, use_flex_ops=False),
    'convert_hlo+flex':
        converters.jax2tflite_fn(HLO_BACKEND, use_flex_ops=True),
}
...
def test_converters():
  ...
  def _clear_error_msg(msg):
    msg = msg.replace('\\n', '\n').replace('\\t', '\t')
    converter_error = 'ConverterError('
    if msg.startswith(converter_error):
      # Converter errors can have extremely long stack traces, but we only care
      # about which ops aren't supported.
      possible_start_strings = [
          'Some ops in the model are custom ops',
          'Some ops are not supported by the native TFLite runtime'
      ]
      for start_str in possible_start_strings:
        if start_str not in msg:
          continue
        i = msg.index(start_str)
        msg = msg[:len(converter_error)+1] + '\n... (CROPPED)...\n' + msg[i:]
        break
    return msg
    ...
    for converter_name, converter_fn in converters.items():
      ...
      try:
        ...
        if FLAGS.fail_on_error:
          raise e
        error_msg = _clear_error_msg(repr(e))
        logging.info(
            '=== ERROR %s',
