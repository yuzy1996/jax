...
def _compare(test_case: ModelTestCase, jax_fn: Callable[..., Any],
  ...
  if len(tf_results) != len(jax_results):
    raise ValueError(f"Returned output tuples lengths do not match: TF length "
                     f"vs JAX length: {len(tf_results)} != {len(jax_results)}")
...
def jax2tf_fn(enable_xla=True):
  """Returns a jax2tf converter function with `enable_xla=enable_xla`."""
  def convert_fn(test_case: ModelTestCase):
    jax_fn = functools.partial(test_case.apply, test_case.variables)
    tf_fn = jax2tf.convert(jax_fn, enable_xla=enable_xla)
    _compare(test_case, jax_fn, tf_fn)
  return convert_fn


def jax2tfjs(test_case: ModelTestCase):
...
def jax2tflite_fn(backend: TFLiteBackend, *, use_flex_ops: bool):
  """Returns a converter using `backend` with Flex ops linked in if `use_flex_ops==True`."""
  def get_converter(apply, input_specs):
    if backend == TFLiteBackend.HLO:
      input_specs = [[(f'inputs{i}', x) for i, x in enumerate(input_specs)]]
      return tf.lite.TFLiteConverter.experimental_from_jax([apply], input_specs)
    else:
      assert backend == TFLiteBackend.JAX2TF
      tf_fn = tf.function(
          jax2tf.convert(apply, enable_xla=False),
          input_signature=_get_signatures(input_specs),
          autograph=False)
      apply_tf = tf_fn.get_concrete_function()
      return tf.lite.TFLiteConverter.from_concrete_functions([apply_tf], tf_fn)

  def convert_fn(test_case):
    apply = functools.partial(test_case.apply, test_case.variables)
    converter = get_converter(apply, test_case.input_specs)
    ...
    _compare(test_case, apply, apply_tflite)
  return convert_fn
